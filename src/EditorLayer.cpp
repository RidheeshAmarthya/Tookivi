...
namespace Tookivi {
	...
	// Load the default tookivi scene when starting the engine
...
#ifdef SN_DEBUG
		serializer.Deserialize("assets/Scenes/Default.tookivi");
#else
		serializer.Deserialize("assets/Scenes/Default_R.tookivi");
#endif // SN_DEBUG
		SceneRenderer::SetScene(m_ActiveScene);
		Application::Get().GetWindow().SetTitle("Tookivi Editor " + m_ActiveScene->m_Name + " scene");
	}
	...
	{
		std::optional<std::string> filepath = FileDialogs::OpenFile("Tookivi Scene (*.tookivi)\0*.tookivi\0");
		if (filepath)
		{
			m_ActiveScene = CreateRef<Scene>();
			m_ActiveScene->OnViewportResize((uint32_t)m_ViewportSize.x, (uint32_t)m_ViewportSize.y);
	...
	{
		std::optional<std::string> filepath = FileDialogs::SaveFile("Tookivi Scene (*.tookivi)\0*.tookivi\0");
		if (filepath)
		{
			SceneSerializer serializer(m_ActiveScene);
