...
namespace Tookivi {
	...
	Tookivi::Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
	{
		// data to fill
	...
	std::vector<texture> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
	{
		...
		for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
		{
			...
			{   // if texture hasn't been loaded already, load it
				...
				Ref<Texture2D> tookiviTexture;
				if (auto tex = m_Scene->GetEmbeddedTexture(str.C_Str())) {
					auto width = tex->mWidth;
					auto height = tex->mHeight;
					tookiviTexture = Texture2D::Create(width,height,reinterpret_cast<unsigned char*>(tex->pcData));
				}
				else
				{
					tookiviTexture = Texture2D::Create(filename);
				}
				if (tookiviTexture) {
					tookiviTextures.push_back(tookiviTexture);
					texture.id = tookiviTexture->GetRendererID();
